...
mapa_total %>%
  ...
  labs(
    ...
    caption = paste0("Fonte: Datasus. Elaboração: VPR/DIESO") 
  )
...
mapa_america_do_sul %>%
  ggplot() +
  geom_sf(data = world , fill = cor_fundo_mapa_pontos) +
  geom_sf(aes(fill= quantidade)) +
  geom_sf(data = world %>%  filter(iso_a2== "BR"), fill = "darkgreen") +
...
mapa_imigrantes %>%
  ...
  geom_curve( data = tibble(long = -8, lat= 39,4),
              ...
              arrow = arrow(length = unit(2, "mm"), type = "closed"),
              color = "white")+
  geom_text(data = tibble(long = 80, lat= 55), aes(x=long, y= lat, label =str_wrap("A média de idade dos pacientes desses quatro países é 67 anos — 1,8x a média considerando todos os países.", 51) ), color= "white", size =3.5, fontface = "bold", vjust=1)+
  scale_fill_continuous_sequential(palette = "heat 2", label = scales::number_format(big.mark = ".")) +
  ...
  labs(
    fill= "Atendimentos",
    title = "Países de imigração histórica somam 3.454 atendimentos no SUS",
    subtitle = "Portugal, Japão, Itália e Alemanha estão entre as 15 nacionalidades mais atendidas",
    caption = paste0("Fonte: Datasus. Elaboração: VPR/DIESO") 
  )
...
sort(unique(sintese_sih_1$uf))

sort(unique(sintese_sih_2$uf))

sort(unique(sintese_sih_3$uf))
